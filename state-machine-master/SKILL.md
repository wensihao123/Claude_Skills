...
/state-machine-master <slug>) — do NOT dive straight into producing or changing artifacts.
The human often wants to brief you first, and shouldn't have to sit through a full work
round just to get a word in. So:
...
- <next> + <slug> MUST match the HANDOFF / state index "下一步" you just wrote — never let them
  drift. (For a STATE-CHANGE that needs implementing, that's /role-planner <slug>; if the change
  touches data models flag /arch-guard, and if it touches screen flow flag /ux-design.)
- If the next baton is another skill, use its command instead (e.g. /design-jam, /image-prompt,
  /arch-guard, /num-smith, /ux-design). If the next step is the human acting outside any role,
  say so plainly and give the command to run AFTER they finish.
- If you only refreshed the standing STATE-MACHINES.md (greenfield/maintenance) with no feature
